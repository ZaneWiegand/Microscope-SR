...
if __name__ == '__main__':
    weights_file = './weight_output_syn/netG_F2_epoch_3.pth'
    upscale_factor = 2
    plus = 1
    number = 84
    print('syn data:')
    ...
    for pic_number in range(number):
        lr_file = '../Data-Post-upsample/10x_predict_syn/10x{}.tif'.format(
            pic_number+plus)
        hr_file = '../Data-Post-upsample/20x_truth/20x{}.tif'.format(
            pic_number+plus)
        ...
        tf.imwrite(
            './pic_output_syn_syn/syn_syn_10x_out{}.tif'.format(pic_number+plus), preds)
    ...
    data_frame.to_csv('test_results_syn_syn.csv', index_label='Epoch')
