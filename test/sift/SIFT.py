    ...
    if len(filter_size) == 2:
        if len(mat_size) == 3:
            ...
            for i in range(mat_size[-1]):
                ...
                for j in range(0, mat_size[0], strides[1]):
                    ...
                    for k in range(0, mat_size[1], strides[0]):
                        val = (
                            filter
                            * pad_mat[j: j + filter_size[0], k: k + filter_size[1]]
                        ).sum()
                        temp[-1].append(val)
            ...
            for j in range(0, mat_size[0], strides[1]):
                ...
                for k in range(0, mat_size[1], strides[0]):
                    val = (
                        filter * pad_mat[j: j + filter_size[0],
                                         k: k + filter_size[1]]
                    ).sum()
...
def adjustLocalExtrema(
    DoG, o, s, x, y, contrastThreshold, edgeThreshold, sigma, n, SIFT_FIXPT_SCALE
):
    ...
    while i < SIFT_MAX_INTERP_STEPS:
        if (
            s < 1
            or s > n
            or y < SIFT_IMG_BORDER
            or y >= img.shape[1] - SIFT_IMG_BORDER
            or x < SIFT_IMG_BORDER
            or x >= img.shape[0] - SIFT_IMG_BORDER
        ):
        ...
        dD = [
            (img[x, y + 1] - img[x, y - 1]) * deriv_scale,
        ...
        dxs = (
            next[x, y + 1] - next[x, y - 1] - prev[x, y + 1] + prev[x, y - 1]
        ) * cross_deriv_scale
        dys = (
            next[x + 1, y] - next[x - 1, y] - prev[x + 1, y] + prev[x - 1, y]
        ) * cross_deriv_scale
    ...
    if (
        det <= 0
        or tr * tr * edgeThreshold >= (edgeThreshold + 1) * (edgeThreshold + 1) * det
    ):
        return None, x, y, s
...
def GetMainDirection(img, r, c, radius, sigma, BinNum):
    ...
    for i in range(BinNum):
        hist.append(
            (temphist[i] + temphist[i + 4]) * (1.0 / 16.0)
            + (temphist[i + 1] + temphist[i + 3]) * (4.0 / 16.0)
        ...
        for s in range(1, S - 1):
            ...
            for i in range(img.shape[0]):
                for j in range(img.shape[1]):
                    val = img[i, j]
                    eight_neiborhood_prev = img_prev[
                        max(0, i - 1): min(i + 2, img_prev.shape[0]),
                        max(0, j - 1): min(j + 2, img_prev.shape[1]),
                    ]
                    eight_neiborhood = img[
                        max(0, i - 1): min(i + 2, img.shape[0]),
                        max(0, j - 1): min(j + 2, img.shape[1]),
                    ]
                    eight_neiborhood_next = img_next[
                        max(0, i - 1): min(i + 2, img_next.shape[0]),
                        max(0, j - 1): min(j + 2, img_next.shape[1]),
                    ]
                    if np.abs(val) > threshold and (
                        (
                            val > 0
                            and (val >= eight_neiborhood_prev).all()
                            and (val >= eight_neiborhood).all()
                            and (val >= eight_neiborhood_next).all()
                        )
                        or (
                            val < 0
                            and (val <= eight_neiborhood_prev).all()
                        ...
                        omax, hist = GetMainDirection(
                            GuassianPyramid[o][layer],
                            x,
                            y,
...
if __name__ == "__main__":
    origimg = plt.imread("./SIFTimg/1-1.jpg")
    ...
    origimg2 = plt.imread("./SIFTimg/1-2.jpg")
    if len(origimg.shape) == 3:
    ...
    img2 = np.array(
        Image.fromarray(img2).resize(
            (int(round(ScaleRatio * img2.shape[1])),
             img.shape[0]), Image.BICUBIC
        )
    )
    ...
    origimg2 = np.array(
        Image.fromarray(origimg2).resize(
